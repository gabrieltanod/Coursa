    ...
    func connect() {
        guard WCSession.isSupported() else {
            ...
            if !session.isCompanionAppInstalled {
                ...
                print(
                    "watchOS: NOTE: In simulators, this check is unreliable. Attempting activation anyway..."
                )
                print("watchOS: Make sure:")
                print("  1. iOS app is running in iOS Simulator FIRST")
                print(
                    "  2. Watch app is launched from 'Watch Coursa via iOS Coursa' scheme"
                )
                print("  3. Both simulators are running the same app")
                print(
                    "watchOS: Proceeding with activation attempt despite warning..."
                )
            } else {
                print(
                    "watchOS: ✅ iOS companion app is installed: \(session.isCompanionAppInstalled)"
                )
            }
            print(
                "watchOS: Activation state before activate(): \(session.activationState.rawValue)"
            )
        #endif
            ...
            if !session.isWatchAppInstalled {
                print(
                    "iOS: WARNING - Watch app is not installed on paired watch"
                )
            }

            print(
                "iOS: Watch app installed: \(session.isWatchAppInstalled), Paired: \(session.isPaired)"
            )
        #endif
    ...
    private func decodeAndStoreSummary(from dictionary: [String: Any]) {
        ...
        guard
            let totalTime = (dictionary["totalTime"] as? Double)
                ?? (dictionary["totalTime"] as? NSNumber)?.doubleValue,
            let totalDistance = (dictionary["totalDistance"] as? Double)
                ?? (dictionary["totalDistance"] as? NSNumber)?.doubleValue,
        ...
        DispatchQueue.main.async {
            ...
            print("📱 iOS: ✅ Summary stored: \(decodedSummary)")
                ...
                if let planSession = self.planSession {
                    print(
                        "📱 iOS: applying summary via PlanSessionStore instance: \(Unmanaged.passUnretained(planSession).toOpaque())"
                    )
                    planSession.apply(summary: decodedSummary)
                } else {
                    print(
                        "📱 iOS: ⚠️ No PlanSessionStore attached to SyncService; summary not applied to plan."
                    )
                }
            #endif
        }
    ...
    // ========================================== MARK: - Send Summary (watchOS to iOS) ==========================================
        ...
        private func sendSummaryData(summary: RunningSummary) {
            ...
            guard session.activationState == .activated else {
                ...
                print(
                    "watchOS: ❌ Cannot send summary - Session is not activated. Current state: \(stateDescription)"
                )
            ...
            print(
                "watchOS: Attempting to send summary (activationState: activated, isReachable: \(session.isReachable))"
            )
            ...
            do {
                ...
                print(
                    "watchOS: ✅ Successfully sent summary via updateApplicationContext"
                )
            } catch {
                print(
                    "watchOS: ❌ Error updating application context: \(error.localizedDescription)"
                )
                ...
                if session.isReachable {
                    ...
                    session.sendMessage(
                        ...
                        replyHandler: { reply in
                            print(
                                "watchOS: ✅ Message sent successfully via sendMessage. Reply: \(reply)"
                            )
                        },
                        errorHandler: { error in
                            print(
                                "watchOS: ❌ Error sending message: \(error.localizedDescription)"
                            )
                    ...
                    print(
                        "watchOS: Session is not reachable. Queueing summary for later..."
                    )
                    pendingSummary = summary
                }
            }
